<template>
    <div class="ReceiptTotal">
        ...
        <el-table :data="datas" border style="width:100%" show-summary :summary-method="getSummaries" @selection-change="selectDatasFun" ref="multipleTable" :row-key="getRowKeys">
            ...
            <el-table-column  prop="sumVal" label="收款总金额(￥)" width="150" >
            </el-table-column>
            <el-table-column v-for="(item,index) in payTypeDatas" :key="index"  :label="item.payName"  width="200" >
                <template slot-scope="scope">
                    {{getids(scope.row.payTypeList,item.payType)}}
                </template>
            </el-table-column>
            <el-table-column f width="150" label="操作">
                <template slot-scope="scope">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            gotoDetail(item){
                this.$router.push({
                    path:"/NavDetail/ReceiptTotaldetail",
                    query:{
            ...
            downSelected(){
                if (this.selectDatas.length === 0) {
                    ...
                    this.payTypeDatas.map(el=>{
                        cateIds.push(el.payType)
                        catevals.push(el.payName)
                    })
                    switch(this.searchFlow.groupType){
                        case "1":
                            let arr1 = ["地区", "部门", "门店", "收款总金额(￥)", ];
                            ...
                            this.payTypeDatas.map(el=>{
                                this.dataTotal.map(val=>{
                                    if(val.payType==el.payType){
                                        newobj[el.payType]=val.val;
                                    }
                                })
                            })
                            this.selectDatas.push(newobj);
                            this.$exportExcel(arr1, Tbody1, this.selectDatas, '收款汇总');
                            break;
                        case "2":
                            let arr2 = [ "部门", "收款总金额(￥)",];
                            ...
                            this.payTypeDatas.map(el=>{
                                this.dataTotal.map(val=>{
                                    if(val.payType==el.payType){
                                        newobj2[el.payType]=val.val;
                                    }
                                })
                            })
                            this.selectDatas.push(newobj2);
                            this.$exportExcel(arr2, Tbody2, this.selectDatas, '收款汇总');
                            break;
                        case "3":
                            let arr3 = [ "地区", "收款总金额(￥)",];
                            ...
                            this.payTypeDatas.map(el=>{
                                this.dataTotal.map(val=>{
                                    if(val.payType==el.payType){
                                        newobj3[el.payType]=val.val;
                                    }
                                })
                            })
                            this.selectDatas.push(newobj3);
                            this.$exportExcel(arr3, Tbody3, this.selectDatas, '收款汇总');
                            break;
                    }
            ...
            demandAll(){
                ...
                this.$post(this.$cspr+"/api/report/paySummary/getPaySummaryData",data).then(d=>{
                    if(d.success){
                        let content=d.data.dataList.content;
                        let totalamount=0;
                        content.map(el=>{
                            el.payTypeList.map(val=>{
                                el[val.payType]=val.val;
                            });
                        ...
                        case "1":
                            ...
                            content.push(newobj);
                            this.$exportExcel(arr1, Tbody1, content, '收款汇总');
                            break;
                        case "2":
                            let arr2 = [ "部门", "收款总金额(￥)",];
                            arr2.push.apply(arr2,catevals);
                            ...
                            this.$exportExcel(arr2, Tbody2, content, '收款汇总');
                            break;
                        case "3":
                            let arr3 = [ "地区", "收款总金额(￥)",];
                            arr3.push.apply(arr3,catevals);
                            ...
                            this.$exportExcel(arr3, Tbody3, content, '收款汇总');
                            break;
                    }
                    }
                })
