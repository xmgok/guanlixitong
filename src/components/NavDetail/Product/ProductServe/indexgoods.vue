        ...
        <div class="zhuti">
            ...
            <header class="funcHeader clearfix">
                <ul class="left">
                    ...
                    <li v-if="exportShow">
                        <!-- <ToImport configKey='impProdAutoCode'></ToImport> -->
                        <Button :Upload="true" Name="导出已选择" @click.native="downSelected"></Button>

                    </li>
                    <li v-if="exportShow">
                        <!-- <Button @click.native="downAll" Name="导出" :Upload="true"/> -->
            ...
            <el-table :data="datas" border style="width:100%" @selection-change="selectDatasFun" :row-key='rowKey'>
                ...
                <el-table-column prop="psName" label="商品名称" width="200"></el-table-column>
                ...
                <el-table-column fixed="right" label="操作" width="250">
                    <template slot-scope="scope" class="">
                        <div class="rtext">
                            ...
                            <Button Name="修改" v-if="editShow" buttonClass="Btn" @click.native="update(scope.row,scope.$index)" />
                            <i class="borderLeft" v-if="scope.row.editShow&&toupdownShow"></i>
                            <Button :Name="scope.row.editName" v-if="scope.row.editShow&&toupdownShow" buttonClass="Btn" @click.native="editFunc(scope.row,scope.$index)" />
...
export default {
    components: {
        ...
        ToImport, ProductCategory: resolve => { require(['../../../common/Product'], resolve) },
    },
    data() {
        return {
            tagArr: [],
            ...
            Tbody: ["psCode", "psName", "psPrice",'paValidStatus', "psStatus"],
            datas: [],
            PageNum: "",
            total: 0,
            codeOrName: "",//产品编号/名称
    ...
    methods: {
        ...
        downSelected() {
            if (this.selectDatas.length === 0) {
                ...
                let arr1 = ["编号", "商品名称", "标准定价",'有效状态', "上架状态",];
                this.$exportExcel(arr1, this.Tbody, this.selectDatas, '产品列表');
        ...
        productAdd() {
            //添加产品服务
            this.$router.push({
                path: "/NavDetail/ProductServe/ProductGoodsAdd"
            })
        ...
        editFunc(item, index) {
            if (item.psStatus == "未上架") {//未上架的立即上架
                this.$post(this.$host + "/api/prod/prtserv/up", { id: item.id }).then(d => {
                    if (d.success) {
                        this.$message({
                            type: "success",
                            message: "该产品已上架成功"
            ...
            } else if (item.psStatus == "已上架") {//已上架的下架
                this.$post(this.$host + "/api/prod/prtserv/down", { id: item.id }).then(d => {
                    if (d.success) {
                        this.$message({
                            type: "success",
                            message: "该产品已下架成功"
        ...
        sureEditFunc() {
            ...
            this.$post(this.$host + "/api/prod/prtserv/up", data).then(d => {
                if (d.success) {
                    this.psOffShelf = "";
                    this.$message({
                        type: "success",
                ...
                if (d.success) {
                    ...
                    if (total) {//导出全部
                        if (content.length === 0) {
                            ...
                            let arr1 = ["编号", "商品名称", "标准定价",'有效状态', "上架状态",];
                            this.$exportExcel(arr1, this.Tbody, content, '产品列表')
                        }
                    } else {
                        this.datas = content;
    ...
    mounted() {
        ...
        auths.map(el => {
            if (el.fParentCode == "F022") {//产品服务的权限
                if (el.fUrl == "add") {
                    this.addShow = true;
                }
                if (el.fUrl == "edit") {
...
.ProductServe .model .footer button {
    margin-right: 24px;
}
