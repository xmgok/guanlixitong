<template>
    <div class="ReceiptTotal">
        ...
        <el-table :data="datas" border style="width:100%" show-summary :summary-method="getSummaries" @selection-change="selectDatasFun" ref="multipleTable" :row-key="getRowKeys">
            ...
            <el-table-column  prop="sumVal" label="退款总金额(￥)" width="200" :key="Math.random()"></el-table-column>
            <el-table-column v-for="(item,index) in cateDatas" :key="index"  :label="item.payName+'(￥)'"  width="200" >
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            gotoDetail(item){
                this.$router.push({
                    path:"/NavDetail/refundTotaldetail",
                    query:{
            ...
            demandAll(){
                ...
                this.$post(this.$cspr+"/api/report/refundSummary/getRefundSummaryData",data).then(d=>{
                    if(d.success){
            ...
            getcates(){
                // 支付类别
                if(this.payTypevuex){
                     this.cateDatas=this.payTypevuex;
                }else{
                    this.$post(this.$cspr+"/api/report/busiSummary/getPayType",{}).then(d=>{
                        if(d.success){
                            let content=d.data;
                            this.cateDatas=content;
                            this.setDatas({name:"payTypevuex",value:this.cateDatas})
                        }
                    })
                }
            },
